...
import { createTrip, editTrip } from "../services/tripService";
  ...
  export default function EditTrip({setShowingPage, setTrips, trips, datosToEdit}){
    ...
    return <Map setShowingPage={setShowingPage} setTrips={setTrips} trips={trips} datosToEdit={datosToEdit}/>
}
...
let dateEdited = (formatDate(dateToEdit));
  ...
  const user = JSON.parse(localStorage.getItem("user"))
    ...
    const [fileUrl, setFileUrl] = useState(`${import.meta.env.VITE_BACKEND_URL}/tripImages/${datosToEdit.image}`)
    ...
    useEffect(()=> {
      ...
      const dateTrip = new Date(date)
       ...
       const date1 = `${dateTrip.getFullYear()}-${dateTrip.getMonth() +1}-${dateTrip.getDate()}`
       console.log(date1)
       ...
       console.log(formJson)
        ...
        const result =  await editTrip(datosToEdit.ID, data.title?data.title:datosToEdit.title, data.date?data.date:date1, data.category?data.category:datosToEdit.category, data.city?data.city:datosToEdit.city, data.excerpt?data.excerpt:datosToEdit.excerpt, data.description?data.description:datosToEdit.description, latitud, longitud )
    ...
    return (
          ...
          <form className="form-container" method="post" onSubmit={handleSubmit(onSubmit)}>

            <div className="img-container">
              <img src={fileUrl} alt="" height="110"/>
              {!fileUrl && (<div className="imagenTrip">
              
              </div>)}
            </div>
      
                 <input type="file" name="file"  onChange={handleOnChange} className="input-img"/>
                 <input type="text" id="city" defaultValue={city}  onChange={handleOnChangeCity}{...register("city",{
              
             
          })}/>
           {errors.city?.type === "required" && <span>Campo requerido</span>}

                <input type="text" id="title" defaultValue={title} onChange={handleOnChangeTitle}{...register("title",{
              
             
          })}/>
           {errors.title?.type === "required" && <span>Campo requerido</span>}
                <select name="category" id="lang" onChange={handleOnChange}  {...register("category",{
                ...
                <input type="text" id="excerpt" defaultValue={excerpt}  onChange={handleOnChangeExcerpt}{...register("excerpt",{
              
             
          })}/>
           {errors.excerpt?.type === "required" && <span>Campo requerido</span>}
                <input type="date" id="date"  defaultValue={date} onChange={handleOnChangeDate} {...register("date",{
              
             
          })}/>
           {errors.date?.type === "required" && <span>Campo requerido</span>}
                <textarea type="text" id="descripcion" defaultValue={description} onChange={handleOnChangeDescription}{...register("description",{
              maxLength: 500,
               ...
               <label>Arrastre la posicion en el mapa</label>
                <GoogleMap zoom={5} center={{lat:parseFloat(datosToEdit.latitude), lng:parseFloat(datosToEdit.longitude)}} mapContainerClassName="map-container" >
          {ok && (<Marker draggable position={{lat: parseFloat(latitud), lng: parseFloat(longitud)}} onDragEnd={coordenadas}/>)}
